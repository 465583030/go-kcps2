...
func (p *DeployVirtualMachineParams) toURLValues() url.Values {
	...
	// It was added for deployPremiumVirtualMachine
	...
	if v, found := p.p["networkids"]; found {
		for i, vv := range v.([]string) {
			// Processing for defining a NetworkIds The parameter ipToNetworkList
			// Example: iptonetworklist[0].networkid=uuid
			u.Set(fmt.Sprintf("iptonetworklist[%d].networkid", i), vv)
		}
		// Description that was originally
		//		vv := strings.Join(v.([]string), ",")
		//		u.Set("networkids", vv)
	}
...
func (p *DeployVirtualMachineParams) SetHostname(v string) {
	if p.p == nil {
		p.p = make(map[string]interface{})
	}
	p.p["hostname"] = v
...
func (s *VirtualMachineService) NewDeployValueVirtualMachineParams(serviceofferingid string, templateid string, zoneid string, hypervisor string, networkids []string, name string) *DeployVirtualMachineParams {
	...
	p.p["networkids"] = networkids
	p.p["name"] = name
	return p
}

// Creates and automatically starts a virtual machine based on a service offering, disk offering, and template.
func (s *VirtualMachineService) DeployValueVirtualMachine(p *DeployVirtualMachineParams) (*DeployVirtualMachineResponse, error) {
...
// as then you are sure you have configured all required params
func (s *VirtualMachineService) NewDeployPremiumVirtualMachineParams(serviceofferingid string, templateid string, zoneid string, hostname string, hypervisor string, networkids []string, name string) *DeployVirtualMachineParams {
	p := &DeployVirtualMachineParams{}
	p.p = make(map[string]interface{})
	p.p["serviceofferingid"] = serviceofferingid
	p.p["templateid"] = templateid
...
// Creates and automatically starts a virtual machine based on a service offering, disk offering, and template.
func (s *VirtualMachineService) DeployPremiumVirtualMachine(p *DeployVirtualMachineParams) (*DeployVirtualMachineResponse, error) {
...
// DeployVirtualMachineResponse was modified for deployValue/PremiumVirtualMachine
type DeployVirtualMachineResponse struct {
	...
	CombinedCPUSpeed      int           `json:"combinedCpuSpeed"`
...
// StartVirtualMachineResponse was modified for startVirtualMachine
